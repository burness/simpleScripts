...
/**
  * Created by burness on 16/4/7.
  */
object MlPipelineDemo {

  def main(args: Array[String]) {
    val file = getClass.getResource("/data_banknote_authentication.txt").toString
    println(file)
    val conf = new SparkConf().setAppName("Classification with ML Pipeline").setMaster("local[1]")
    ...
    /** Step 1
      ...
      * ... ...
      */
    val parsedRDD = sc.textFile(file).map(_.split(",")).map(eachRow => {
